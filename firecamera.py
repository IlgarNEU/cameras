...
# ── Configuration ─────────────────────────────────────────────────────────────
RTSP_URL       = "rtsp://192.168.33.253:8554/s-tvfeed-9"
OUTPUT_DIR     = os.path.expanduser("~/Desktop/recordings/fire")
...
log_path = os.path.join(OUTPUT_LOG_DIR, f"firerecorder_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
...
def make_filename() -> str:
    ...
    return os.path.join(OUTPUT_DIR, f"firestream_{ts}.mkv")
